...
const drawInputByTextSchema = (arg: {
  ...
  input.split(/\r|\n|\r\n/g).forEach((inputLine, inputLineIndex) => {
    ...
    let splitedLines = getSplittedLines(inputLine, isOverEval);
    const maxLines = Math.floor(height / (fontValue.heightAtSize(size) * lineHeight));

    if (splitedLines.length > maxLines) {
      splitedLines = splitedLines.slice(0, maxLines - 1);
      splitedLines.push(splitedLines.pop() + '...');
    }
